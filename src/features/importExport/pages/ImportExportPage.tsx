...
export function ImportExportPage() {
  ...
  return (
    <div>
      ...
      <FormModal open={editOpen} title={editingJob ? `Editar ${editingKind === 'import' ? 'importación' : 'exportación'} ${editingJob.code}` : 'Editar job'} onClose={() => { setEditOpen(false); setEditingJob(null); }} onSubmit={(e) => { e.preventDefault(); if (!editingJob) return; if (editingKind === 'import') updateImport.mutate({ id: editingJob.id, body: editForm }); else updateExport.mutate({ id: editingJob.id, body: editForm }); }} loading={updateImport.isPending || updateExport.isPending} submitLabel="Guardar cambios">
